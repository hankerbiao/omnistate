...
python scripts/init/sync_workflow.py      # Upsert workflow configs (no deletion)
python scripts/init/sync_rbac.py          # Sync RBAC roles
...
- Beanie index sync is skipped on application startup by default (`SKIP_INDEX_SYNC=1`). Run `python scripts/init/sync_indexes.py` to synchronize all indexes explicitly.
