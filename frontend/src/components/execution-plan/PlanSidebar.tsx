...
export default function PlanSidebar({ plans, activePlanId, loading, searchQuery, onSelect }: PlanSidebarProps) {
  return (
    <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid var(--border-subtle)', background: 'var(--surface-primary)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                  <div style={{ flex: 1, height: 3, background: 'var(--surface-tertiary)', borderRadius: 2, overflow: 'hidden' }}>
                    <div style={{ width: `${p.progress_percent ?? 0}%`, height: '100%', background: p.status === 'active' ? 'var(--accent-primary)' : '#8b949e', borderRadius: 2, transition: 'width 0.3s' }} />
                  </div>
