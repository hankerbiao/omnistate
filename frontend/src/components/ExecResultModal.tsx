...
interface Props {
  taskId: string;
  loading: boolean;
  data: ExecResultData | null;
  onClose: () => void;
}

export interface ExecResultCase {
  case_id?: string;
  auto_case_id?: string;
  title?: string;
  status?: string;
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', fontSize: 12 }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {data?.cases?.map((c, i) => {
              const assertionCount = c.result_data?.assertions?.length ?? 0;
              return (
                <div key={c.case_id || i} style={{
                  ...
                  {assertionCount > 0 && (
                    <div style={{ marginTop: 4 }}>
                      <span style={{ fontSize: 10, color: 'var(--text-tertiary)' }}>断言: {assertionCount} 条</span>
                    </div>
                  )}
                </div>
              );
