...
export const ReqList: React.FC<ReqListProps> = ({
  ...
  const filterSections = useMemo((): FilterSection[] => {
    ...
    const priorityOptions = Object.values(Priority).map(priority => ({
      value: priority,
      label: priority === Priority.P0 ? 'P0 - 紧急' : priority === Priority.P1 ? 'P1 - 高' : 'P2 - 普通',
      count: requirements.filter(req => req.priority === priority).length,
    }));
  ...
  const filteredRequirements = useMemo(() => {
    return requirements.filter(req => {
      ...
      // Status filter
      if (selectedFilters.status.size > 0 && !selectedFilters.status.has(req.status)) {
        return false;
      }

      // Priority filter
      if (selectedFilters.priority.size > 0 && !selectedFilters.priority.has(req.priority)) {
        return false;
      }
  ...
  }, [requirements, searchText, selectedFilters]);
          ...
          {hasActiveFilters && (
            <div className="p-4 border-t border-slate-100 bg-slate-50">
              <div className="text-xs text-slate-500 text-center">
                找到 <span className="font-bold text-indigo-600">{filteredRequirements.length}</span> 个测试需求
              </div>
            </div>
          )}
        </aside>
      ...
      <main className="flex-1 p-8 space-y-8 overflow-auto">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-900 tracking-tight">测试需求看板</h1>
              <p className="text-base text-slate-500 mt-2">管理硬件验证需求、技术规范及测试覆盖情况</p>
            </div>
          </div>
          <button
            onClick={onCreateReq}
            className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 transition-all shadow-xl hover:shadow-2xl shadow-slate-900/20 active:scale-95"
          >
            <Plus size={20} /> 创建测试需求
          </button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-wider">ID</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredRequirements.map(req => (
                <tr key={req.req_id} className="hover:bg-slate-50/80 transition-colors cursor-pointer group" onClick={() => onSelectReq(req)}>
                  <td className="px-8 py-5 font-mono text-sm font-bold text-indigo-600">{req.req_id}</td>
                  ...
                  <td className="px-8 py-5">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full border ${
                      req.priority === Priority.P0 ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-amber-50 text-amber-600 border-amber-100'
                    }`}>{req.priority}</span>
                  </td>
                  <td className="px-8 py-5">
