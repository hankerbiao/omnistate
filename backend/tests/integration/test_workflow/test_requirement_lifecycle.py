...
@pytest.mark.asyncio
async def test_requirement_full_lifecycle(client_admin: AsyncClient):
    ...
    # 4.3 DRAFT → SUBMIT → PENDING_REVIEW
    resp = await client_admin.post(
    ...
    # 4.4 PENDING_REVIEW → APPROVE → PENDING_DEVELOP (admin can do it)
    resp = await client_admin.post(
        f"/api/v1/work-items/{item_id}/transition",
        json=create_transition_request("APPROVE", {"target_owner_id": "test_admin", "comment": "approved"}),
    )
    assert resp.status_code == 200, f"APPROVE failed: {resp.text}"
    assert resp.json()["data"]["to_state"] == "PENDING_DEVELOP"

    # 4.5 PENDING_REVIEW → REJECT → DRAFT (alternative path)
    # Already approved above, so skip this test case in the full lifecycle

    # 4.6 PENDING_DEVELOP → START → DEVELOPING
    resp = await client_admin.post(
        f"/api/v1/work-items/{item_id}/transition",
        json=create_transition_request("START", {"target_owner_id": "test_admin"}),
    )
    ...
    resp = await client_admin.post(
        f"/api/v1/work-items/{item_id}/transition",
        json=create_transition_request("FINISH", {"target_owner_id": "test_admin"}),
    )
    ...
    resp = await client_admin.post(
        f"/api/v1/work-items/{item_id}/transition",
        json=create_transition_request("PASS", {"target_owner_id": "test_admin"}),
    )
    assert resp.status_code == 200, f"PASS failed: {resp.text}"
    assert resp.json()["data"]["to_state"] == "PENDING_UAT"

    # 4.9 PENDING_TEST → REJECT → DEVELOPING (alternative path)
    # Already passed above, so skip
