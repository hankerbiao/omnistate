...
2. **priority** ↔ **Requirement.priority**: 测试用例优先级，建议继承或不超过对应需求的优先级级别
3. **attachments** ↔ **Requirement.attachments**: 附件列表结构相同，测试用例可继承需求的附件或新增相关附件
...
class AutomationTestCaseResponse(BaseModel):
    ...
    linked_manual_case_id: Optional[str] = Field(None, description="关联的平台手工测试用例编号（null=未关联）")
    name: str = Field(..., description="自动化用例名称")
    ...
    repo_branch: Optional[str] = Field(None, description="仓库分支")
    script_ref: Dict[str, Any] = Field(default_factory=dict, description="脚本定位信息")
    ...
    entry_command: Optional[str] = Field(None, description="入口命令")
    runtime_env: Dict[str, Any] = Field(default_factory=dict, description="运行环境配置")
    code_snapshot: Dict[str, Any] = Field(default_factory=dict, description="代码版本快照")
    ...
    reviewer_id: Optional[str] = Field(None, description="审核人")
