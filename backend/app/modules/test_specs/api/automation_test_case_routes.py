...
@router.get(
    "/by-manual-case-id/{linked_manual_case_id}",
    response_model=APIResponse[AutomationTestCaseResponse],
...
async def get_automation_test_case_by_manual_case_id(
    ...
    """按 linked_manual_case_id 查询最新一条上报记录。"""
    try:
        data = await service.get_automation_test_case_by_manual_case_id(linked_manual_case_id)
        return APIResponse(data=data)
    except KeyError:
        raise HTTPException(status_code=404, detail="automation test case not found")
