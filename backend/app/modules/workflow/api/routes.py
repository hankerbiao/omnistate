...
async def get_work_types(service: WorkflowServiceDep):
    ...
    - 配置页面中展示支持的事项类型
    """
    data = await service.get_work_types()
...
async def get_workflow_states(service: WorkflowServiceDep):
    ...
    - 构建看板列 / 流程配置界面
    """
    data = await service.get_workflow_states()
    return APIResponse(data=data)
...
async def search_work_items(
    ...
    - 其余筛选条件与 list_work_items 一致
    """
    data = await service.search_items(
        keyword=keyword,
        type_code=type_code,
        state=state,
        owner_id=owner_id,
...
async def list_test_cases_for_requirement(
    ...
    """
    try:
        data = await service.list_test_cases_for_requirement(item_id)
        return APIResponse(data=data)
...
async def get_requirement_for_test_case(
    ...
    """
    try:
        data = await service.get_requirement_for_test_case(item_id)
        return APIResponse(data=data)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_transition_logs(
    ...
    """
    try:
        data = await service.get_logs(item_id, limit)
        return APIResponse(data=data)
    except WorkItemNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
