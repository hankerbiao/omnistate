...
`auth` 模块负责用户认证、用户/角色管理和基于静态权限码的 RBAC 授权。
...
工作流规则定义在 `backend/app/configs/*.json`，通过 `backend/scripts/init/sync_workflow.py` 同步到 MongoDB。
...
2. 运行 `python scripts/init/sync_workflow.py`。
